...
- Bare hostile spell shortcuts use the current target first, then fall back to the built-in battle target token `<bt>` when no current target is selected; explicit user-provided target tokens override both defaults.
...
- hostile spell name list used to choose current target or `<bt>` as the default target
